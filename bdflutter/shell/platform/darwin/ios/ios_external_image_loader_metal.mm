...
IOSMetalExternalImageLoader::IOSMetalExternalImageLoader(NSObject<FlutterImageLoader>* imageLoader)
    : IOSExternalImageLoader(imageLoader) {
...
void IOSMetalExternalImageLoader::ImageLoaderCallback(
    IOSImageInfo imageInfo,
    std::shared_ptr<ImageLoaderCallbackContext<void(sk_sp<SkImage> image)>>
        imageloaderCallbackContext) {
  std::function<void(sk_sp<SkImage> image)> callback =
      std::move(imageloaderCallbackContext->callback);
  ...
  if (!cache_ref_) {
    ...
    auto cv_return =
        CVMetalTextureCacheCreate(kCFAllocatorDefault, NULL, device_.get(), NULL, &cache);
  ...
  if (cache_ref_) {
    ...
    if (bufferRef != nullptr) {
      auto cv_return = CVMetalTextureCacheCreateTextureFromImage(
          kCFAllocatorDefault, cache_ref_, bufferRef, NULL, MTLPixelFormatBGRA8Unorm,
          static_cast<int>(CVPixelBufferGetWidth(bufferRef)),
          static_cast<int>(CVPixelBufferGetHeight(bufferRef)), 0u, &texture);
      if (cv_return != kCVReturnSuccess) {
  ...
  if (!texture) {
    io_task_runner->PostTask(
        fml::MakeCopyable([callback = std::move(callback)]() mutable { callback(nullptr); }));
  } else {
    ...
    io_task_runner->PostTask(fml::MakeCopyable(
        [context = std::move(context), texture = std::move(texture), callback = std::move(callback),
         backendTexture = std::move(backendTexture)]() mutable {
          ...
          sk_sp<SkImage> image = SkImage::MakeFromTexture(
              context.get(), backendTexture, kTopLeft_GrSurfaceOrigin, kBGRA_8888_SkColorType,
...
void IOSMetalExternalImageLoader::Load(const std::string url,
                                       const int width,
                                       const int height,
                                       const float scale,
                                       ImageLoaderContext loaderContext,
                                       std::function<void(sk_sp<SkImage> image)> callback) {
  NSString* urlStr = [NSString stringWithCString:url.c_str()
  ...
  std::shared_ptr<ImageLoaderCallbackContext<void(sk_sp<SkImage> image)>>
      imageLoaderCallbackContext =
          std::make_shared<ImageLoaderCallbackContext<void(sk_sp<SkImage> image)>>(
              std::move(callback));
...
void IOSMetalExternalImageLoader::LoadCodec(
  ...
  const auto& task_runners = loaderContext.task_runners;
  auto io_task_runner = task_runners.GetIOTaskRunner();
  std::shared_ptr<ImageLoaderCallbackContext<void(std::unique_ptr<NativeExportCodec> codec)>>
      imageLoaderCallbackContext = std::make_shared<
          ImageLoaderCallbackContext<void(std::unique_ptr<NativeExportCodec> codec)>>(callback);
  void (^complete)(NSObject<IOSImageCodec>*) = ^(NSObject<IOSImageCodec>* iOSCodec) {
    std::function<void(std::unique_ptr<NativeExportCodec> codec)> contextCallback =
        std::move(imageLoaderCallbackContext->callback);
    imageLoaderCallbackContext->callback = nullptr;
    auto codec = std::make_unique<IOSNativeExportCodec>(iOSCodec);

    io_task_runner->PostTask(
        fml::MakeCopyable([callback = std::move(contextCallback),
                           codec = std::move(codec)]() mutable { callback(std::move(codec)); }));
  };
...
void IOSMetalExternalImageLoader::GetNextFrame(
    ImageLoaderContext loaderContext,
    int currentFrame,
    std::shared_ptr<NativeExportCodec> codec,
    std::function<void(sk_sp<SkImage> skimage)> callback) {
  ...
  std::shared_ptr<ImageLoaderCallbackContext<void(sk_sp<SkImage> skimage)>>
      imageLoaderCallbackContext =
          std::make_shared<ImageLoaderCallbackContext<void(sk_sp<SkImage> skimage)>>(callback);
  imageLoaderCallbackContext->io_task_runner = io_task_runner;
