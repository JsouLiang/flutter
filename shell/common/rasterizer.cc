...
RasterStatus Rasterizer::DrawToSurface(flutter::LayerTree& layer_tree) {
  ...
  if (surface_->AllowsDrawingWhenGpuDisabled()) {
    raster_status = DrawToSurfaceUnsafe(layer_tree);
  } else {
    delegate_.GetIsGpuDisabledSyncSwitch()->Execute(
        fml::SyncSwitch::Handlers()
            ...
            .SetIfFalse([&] {
              raster_status =
                  DrawToSurfaceUnsafe(layer_tree);
            }));
  }
...
RasterStatus Rasterizer::DrawToSurfaceUnsafe(
  ...
  // BD ADD: START
  fml::TimeDelta build_time = layer_tree.build_time();
  int miss_count = (int) (build_time.ToMillisecondsF() / delegate_.GetFrameBudget().count());
