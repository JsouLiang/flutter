...
+ (void)TraceApmStartAndEnd:(NSString*)event timestamp:(NSNumber*)timestamp {
  flutter::Performance::GetInstance()->TraceApmStartAndEnd(
      static_cast<std::string>([event UTF8String]), [timestamp longLongValue]);
}
// END
...
- (BOOL)createShell:(NSString*)entrypoint
  ...
  // BD ADD:
  // to report ber_shell_create cost, need a start time, use android apm native_init event, see
  // android code start time arg ignore
