...
AndroidNativeExportCodec::AndroidNativeExportCodec(JNIEnv* env,
                                                   std::string cKey,
                                                   jobject jCodec) {
  ...
  jint frameCount =
      env->GetIntField(jCodec, env->GetFieldID(clazz, "frameCount", "I"));
  jint repeatCount =
      env->GetIntField(jCodec, env->GetFieldID(clazz, "repeatCount", "I"));
  jobject jObject = env->GetObjectField(
      jCodec, env->GetFieldID(clazz, "codec", "Ljava/lang/Object;"));
  int* frameDurations = nullptr;
  jobject frameDurationsObj = env->GetObjectField(
      jCodec, env->GetFieldID(clazz, "frameDurations", "[I"));
  if (frameDurationsObj) {
    jintArray* frameDurationsSrc =
        reinterpret_cast<jintArray*>(&frameDurationsObj);
    jsize frameDurationsSize = env->GetArrayLength(*frameDurationsSrc);
    if (frameDurationsSize > 0) {
      frameDurations =
          static_cast<int*>(malloc(sizeof(int) * frameDurationsSize));
      env->GetIntArrayRegion(*frameDurationsSrc, 0, frameDurationsSize,
                             frameDurations);
    }
  }
