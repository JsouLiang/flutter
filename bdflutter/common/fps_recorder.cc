...
std::vector<double> FpsRecorder::ObtainFpsData(const std::string& key,
                                               bool stopRecord) {
  draw_lock.lock();
  ...
  if (it != fps_data_.end()) {
    ...
    if (fps_value.first == 0) {  // frame count is 0
      ...
      result[0] = (fps_value.second + (is_drawn ? 0 : 1)) *
                  kFrameCountOneSecond / fps_value.first;  // Fps value
      if (ui_time_[key].first == 0) {                      // count == 0
        result[1] = 0;
      } else {
        result[1] = ui_time_[key].second * 1.0 / ui_time_[key].first /
                    1000.0;  // convert to millSeconds
      }
