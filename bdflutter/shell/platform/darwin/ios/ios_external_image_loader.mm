...
struct ImageLoaderCallbackContext {
  ...
  ImageLoaderCallbackContext(const TaskRunners& task_runners)
      : callback(nullptr), task_runners(task_runners) {}
...
void IOSExternalImageLoader::Load(const std::string url,
                                  const int width,
                                  const int height,
                                  const float scale,
  ...
  std::shared_ptr<ImageLoaderCallbackContext> imageLoaderCallbackContext =
      std::make_shared<ImageLoaderCallbackContext>(task_runners);
  ...
  void (^complete)(IOSImageInfo) = ^(IOSImageInfo imageInfo) {
    ...
    if (!cache_ref_) {
      ...
      CVReturn err =
          CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL, glcontext, NULL, &cache);
      if (err == noErr) {
    ...
    if (!texture) {
      ...
      io_task_runner->PostTask(fml::MakeCopyable(
          [context = std::move(context), texture = std::move(texture),
           callback = std::move(callback), backendTexture = std::move(backendTexture)]() mutable {
