...
IOSOpenGLExternalImageLoader::IOSOpenGLExternalImageLoader(
    NSObject<FlutterImageLoader>* imageLoader)
...
void IOSOpenGLExternalImageLoader::ImageLoaderCallback(
        ...
        [context = std::move(context), texture = std::move(texture), callback = std::move(callback),
         backendTexture = std::move(backendTexture)]() mutable {
          ...
          sk_sp<SkImage> image = SkImage::MakeFromTexture(
              context.get(), backendTexture, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
...
void IOSOpenGLExternalImageLoader::Load(const std::string url,
                                        const int width,
                                        const int height,
                                        const float scale,
                                        ImageLoaderContext loaderContext,
                                        std::function<void(sk_sp<SkImage> image)> callback) {
  NSString* URL = [NSString stringWithCString:url.c_str()
...
void IOSOpenGLExternalImageLoader::LoadCodec(
...
void IOSOpenGLExternalImageLoader::GetNextFrame(
    ImageLoaderContext loaderContext,
    int currentFrame,
