...
class IOSOpenGLExternalImageLoader : public IOSExternalImageLoader {
 public:
  ...
  void LoadCodec(const std::string url,
                 ...
                 std::function<void(std::unique_ptr<NativeExportCodec> codec)> callback) override;

  void GetNextFrame(ImageLoaderContext loaderContext,
                    int currentFrame,
                    std::shared_ptr<NativeExportCodec> codec,
                    std::function<void(sk_sp<SkImage> skimage)> callback) override;
 ...
 private:
  ...
  void ImageLoaderCallback(IOSImageInfo imageInfo,
