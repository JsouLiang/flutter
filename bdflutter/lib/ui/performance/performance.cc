...
void Performance_obtainFps(Dart_NativeArguments args) {
  ...
  bool stop_record =
      tonic::DartConverter<bool>::FromArguments(args, 2, exception);
  if (exception) {
    Dart_ThrowException(exception);
    return;
  }
  std::vector<double> fpsInfo =
      FpsRecorder::Current()->ObtainFpsData(key, stop_record);
  int size = 3;
...
void Performance_startBoost(Dart_NativeArguments args) {
  uint16_t flags = static_cast<uint16_t>(
      tonic::DartConverter<int>::FromDart(Dart_GetNativeArgument(args, 1)));
  int millis =
...
#if defined(THIRD_PARTY_DART_BD)
  int64_t microseconds = (int64_t)DartConverter<int64_t>::FromDart(
...
void Performance::GetGpuCacheUsageKB(int64_t* grTotalMem,
  ...
  if (rasterizer_ && rasterizer_.get()) {
    ...
    rasterizer_->getResourceCacheBytes(&totalBytes, &resourceBytes,
                                       &purgeableBytes);
#endif
#endif
...
void Performance::GetIOGpuCacheUsageKB(int64_t* grTotalMem,
  ...
  if (iOManager_ && iOManager_.get() && iOManager_->GetResourceContext()) {
    ...
    size_t resourceBytes = 0;
    size_t purgeableBytes = 0;
    iOManager_->GetResourceContext()->getResourceCacheBytes(
        &totalBytes, &resourceBytes, &purgeableBytes);
    *grTotalMem = totalBytes >> 10;
    *grResMem = resourceBytes >> 10;
    *grPurgeableMem = purgeableBytes >> 10;
  }
...
void Performance_skGraphicCacheMemoryUsage(Dart_NativeArguments args) {
  ...
  Performance::GetInstance()->GetSkGraphicMemUsageKB(&bitmapMem, &fontMem,
                                                     &imageFilter, &mallocSize);
  ...
  task_runners.GetIOTaskRunner()->PostTask(fml::MakeCopyable(
      [callback = std::make_unique<tonic::DartPersistentValue>(
       ...
       gpu_task_runner = task_runners.GetRasterTaskRunner()]() mutable {
        ...
        Performance::GetInstance()->GetIOGpuCacheUsageKB(
            &iOGrTotalMem, &iOGrResMem, &iOGrPurgeableMem);
        ...
        gpu_task_runner->PostTask(fml::MakeCopyable(
            [callback = std::move(callback), ui_task_runner, iOGrTotalMem,
             iOGrResMem, iOGrPurgeableMem]() mutable {
              if (ui_task_runner.get()) {
                ...
                Performance::GetInstance()->GetGpuCacheUsageKB(
                    &grTotalMem, &grResMem, &grPurgeableMem);
                ui_task_runner->PostTask(fml::MakeCopyable(
                    [callback = std::move(callback), grTotalMem, grResMem,
                     grPurgeableMem, iOGrTotalMem, iOGrResMem,
                     iOGrPurgeableMem]() {
                      std::shared_ptr<tonic::DartState> dart_state =
                          callback->dart_state().lock();
                      if (!dart_state || !dart_state.get() ||
                          Performance::GetInstance()->IsExitApp()) {
                        callback->Clear();
                        return;
                      }
                      tonic::DartState::Scope scope(dart_state);
                      Dart_Handle data_handle = Dart_NewList(6);
                      Dart_ListSetAt(data_handle, 0,
                                     Dart_NewInteger(grTotalMem));
                      Dart_ListSetAt(data_handle, 1, Dart_NewInteger(grResMem));
                      Dart_ListSetAt(data_handle, 2,
                                     Dart_NewInteger(grPurgeableMem));
                      Dart_ListSetAt(data_handle, 3,
                                     Dart_NewInteger(iOGrTotalMem));
                      Dart_ListSetAt(data_handle, 4,
                                     Dart_NewInteger(iOGrResMem));
                      Dart_ListSetAt(data_handle, 5,
                                     Dart_NewInteger(iOGrPurgeableMem));
...
void Performance_getTotalExtMemInfo(Dart_NativeArguments args) {
  ...
  task_runners.GetIOTaskRunner()->PostTask(fml::MakeCopyable(
      [callback = std::make_unique<tonic::DartPersistentValue>(
           tonic::DartState::Current(), callback_handle),
       ui_task_runner = task_runners.GetUITaskRunner(),
       gpu_task_runner = task_runners.GetRasterTaskRunner(), imageMem,
       bitmapMem, fontMem, imageFilter, mallocSize]() mutable {
        if (!gpu_task_runner.get() || Performance::GetInstance()->IsExitApp()) {
          ui_task_runner->PostTask(fml::MakeCopyable(
              [callback = std::move(callback)]() { callback->Clear(); }));
          return;
        ...
        Performance::GetInstance()->GetIOGpuCacheUsageKB(
            &iOGrTotalMem, &iOGrResMem, &iOGrPurgeableMem);
        gpu_task_runner->PostTask(fml::MakeCopyable([callback =
                                                         std::move(callback),
                                                     ui_task_runner, imageMem,
                                                     bitmapMem, fontMem,
                                                     imageFilter, iOGrTotalMem,
                                                     iOGrResMem,
                                                     iOGrPurgeableMem,
                                                     mallocSize]() mutable {
          if (ui_task_runner.get()) {
            ...
            Performance::GetInstance()->GetGpuCacheUsageKB(
                &grTotalMem, &grResMem, &grPurgeableMem);
            ui_task_runner->PostTask(fml::MakeCopyable(
                [callback = std::move(callback), imageMem, bitmapMem, fontMem,
                 imageFilter, grTotalMem, grResMem, grPurgeableMem,
                 iOGrTotalMem, iOGrResMem, iOGrPurgeableMem, mallocSize]() {
                  std::shared_ptr<tonic::DartState> dart_state =
                      callback->dart_state().lock();
                  if (!dart_state || !dart_state.get() ||
                      Performance::GetInstance()->IsExitApp()) {
...
void Performance::SetRasterizerAndIOManager(
    fml::TaskRunnerAffineWeakPtr<flutter::Rasterizer> rasterizer,
...
#if defined(OS_IOS) || defined(OS_ANDROID)
  rasterizer->getResourceCacheBytes(&totalBytes, &resourceBytes,
                                    &purgeableBytes);
#endif
...
void Performance::UpdateIOCacheUsageKB(
    fml::WeakPtr<GrDirectContext> iOContext) {
#if THIRD_PARTY_SKIA_BD
#if defined(OS_IOS) || defined(OS_ANDROID)
  if (iOContext) {
    size_t totalBytes = 0;
    size_t resourceBytes = 0;
    size_t purgeableBytes = 0;
    iOContext->getResourceCacheBytes(&totalBytes, &resourceBytes,
                                     &purgeableBytes);
...
void Performance::RegisterNatives(tonic::DartLibraryNatives* natives) {
  natives->Register({
      ...
      {"Performance_getEngineMainEnterMicros",
       Performance_getEngineMainEnterMicros, 1, true},
      {"Performance_addNextFrameCallback", Performance_addNextFrameCallback, 2,
       true},
      ...
      {"Performance_startStackTraceSamples", Performance_startStackTraceSamples,
       1, true},
      {"Performance_stopStackTraceSamples", Performance_stopStackTraceSamples,
       1, true},
      {"Performance_getStackTraceSamples", Performance_getStackTraceSamples, 2,
       true},
      // {"Performance_requestHeapSnapshot", Performance_requestHeapSnapshot, 2,
      // true},
      // {"Performance_heapInfo", Performance_heapInfo, 1, true},
      {"Performance_getEngineInitApmInfo", Performance_getEngineInitApmInfo, 1,
       true},
      {"Performance_getTotalExtMemInfo", Performance_getTotalExtMemInfo, 2,
       true},
      {"Performance_skGraphicCacheMemoryUsage",
