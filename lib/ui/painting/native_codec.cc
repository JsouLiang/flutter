        ...
        [callback = std::make_unique<DartPersistentValue>(
         ...
         io_manager = dart_state->GetIOManager()]() mutable {
          ...
          state->GetNextFrameAndInvokeCallback(
              ...
              trace_id, state->codec_->skImage, duration);
        }));
  } else {
    task_runners.GetIOTaskRunner()->PostTask(fml::MakeCopyable(
        [dart_state, trace_id, io_manager = dart_state->GetIOManager(),
         ...
         callback = std::make_unique<DartPersistentValue>(
          ...
          ImageLoaderContext loaderContext = ImageLoaderContext(
              task_runners, io_manager->GetResourceContext());
          io_manager->GetImageLoader()->GetNextFrame(
              loaderContext, state->nextFrameIndex_, state->codec_,
              fml::MakeCopyable(
                  [context = dart_state->GetResourceContext(),
                   ...
                   trace_id](sk_sp<SkImage> skimage) mutable {
                    ...
                    state->GetNextFrameAndInvokeCallback(
                        ...
                        io_manager->GetSkiaUnrefQueue(), trace_id, skimage,
                        duration);
                  }));
        }));
  }
