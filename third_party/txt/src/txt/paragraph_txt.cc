...
bool ParagraphTxt::ComputeLineBreaks() {
  ...
  for (size_t newline_index = 0; newline_index < newline_positions.size();
    ...
    while (run_index < runs_.size()) {
      ...
      minikin::MinikinPaint paint;
      // BD ADD:
      paint.scaleMeasureByDensity = paragraph_style_.drawMinHeight ||
                                    paragraph_style_.forceVerticalCenter;
...
void ParagraphTxt::Layout(double width) {
  ...
  for (size_t line_number = 0; line_number < line_limit; ++line_number) {
    ...
    bool shouldTextBounds =
        paragraph_style_.drawMinHeight || paragraph_style_.forceVerticalCenter;
    // END
    for (auto line_run_it = line_runs.begin(); line_run_it != line_runs.end();
      ...
      if (shouldTextBounds) {
        ...
        rectForHeight.mBottom =
            std::max(rectForHeight.mBottom, subRect.mBottom);
      }
      // END
    ...
    shouldTextBounds = shouldTextBounds && rectForHeight.mTop != FLT_MAX &&
                       rectForHeight.mBottom != -FLT_MAX;
    if (shouldTextBounds) {
      if (paragraph_style_.drawMinHeight) {
        ...
        double leading =
            (height - (-rectForHeight.mTop + rectForHeight.mBottom)) / 2.0;
